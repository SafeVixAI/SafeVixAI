...
describe('Modal', function() {
  it('returns null when not open', function() {
    const c = render(React.createElement(Modal, { open: false, onClose: jest.fn(), title: 'Test' }, null))
  ...
  it('does not render footer when absent', function() {
    const c = render(React.createElement(Modal, { open: true, onClose: jest.fn(), title: 'Test' }, null))
    const footers = c.container.querySelectorAll('[class*="border-t"]')
  ...
  it('does not call onClose when panel clicked', function() {
    const onClose = jest.fn()
    const c = render(React.createElement(Modal, { open: true, onClose: onClose, title: 'Test' }, null))
    const panel = c.container.querySelector('[class*="max-w"]')
  ...
  it('renders with sm size classes', function() {
    const c = render(React.createElement(Modal, { open: true, onClose: jest.fn(), title: 'SM', size: 'sm' }, null))
    expect(c.container.querySelector('.max-w-sm')).toBeInTheDocument()
  })

  it('renders with lg size classes', function() {
    const c = render(React.createElement(Modal, { open: true, onClose: jest.fn(), title: 'LG', size: 'lg' }, null))
    expect(c.container.querySelector('.max-w-lg')).toBeInTheDocument()
  })

  it('renders with md size by default', function() {
    const c = render(React.createElement(Modal, { open: true, onClose: jest.fn(), title: 'MD' }, null))
