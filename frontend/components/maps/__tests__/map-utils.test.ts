...
describe('map-utils', function () {
  ...
  describe('buildFacilityCollection', function () {
    ...
    it('returns a FeatureCollection', function () {
      const result = utils.buildFacilityCollection(facilities, null)
    ...
    it('sets selected property to 1 for selected facility', function () {
      const result = utils.buildFacilityCollection(facilities, 'f1')
      const f1 = result.features.find(function(f) { return f.properties.id === 'f1' })
      expect(f1.properties.selected).toBe(1)
      const f2 = result.features.find(function(f) { return f.properties.id === 'f2' })
    ...
    it('filters out facilities without coords', function () {
      const bad = { id: 'f3', name: 'Bad', type: 'unknown', coords: null, accentColor: '#fff' }
      const result = utils.buildFacilityCollection(facilities.concat([bad]), null)
      expect(result.features).toHaveLength(2)
    })
    ...
    it('uses custom icon when provided', function () {
      const withIcon = [{ id: 'f3', name: 'Custom', type: 'school', coords: [12, 81], accentColor: '#fff', icon: 'X' }]
      const result = utils.buildFacilityCollection(withIcon, null)
  ...
  describe('buildAccuracyFeature', function () {
    it('returns a Polygon Feature', function () {
      const result = utils.buildAccuracyFeature(13.0, 80.0, 100)
    ...
    it('returns different shape for different accuracy', function () {
      const small = utils.buildAccuracyFeature(13.0, 80.0, 10)
      const large = utils.buildAccuracyFeature(13.0, 80.0, 1000)
  ...
  describe('buildMarkerElement', function () {
    it('returns an HTMLDivElement', function () {
      const el = utils.buildMarkerElement({ color: '#00c896', icon: 'H', kind: 'standard' })
    ...
    it('sets aria-label for current kind', function () {
      const el = utils.buildMarkerElement({ color: '#2563eb', icon: '', kind: 'current' })
      expect(el.getAttribute('aria-label')).toContain('Your current location')
    })

    it('sets aria-label for issue kind', function () {
      const el = utils.buildMarkerElement({ color: '#ef4444', icon: '!', kind: 'issue' })
    ...
    it('applies selected transform when selected is true', function () {
      const el = utils.buildMarkerElement({ color: '#00c896', icon: 'H', kind: 'standard', selected: true })
      const shell = el.firstChild
      expect(shell.style.transform).toContain('scale(1.08)')
    })

    it('does not apply selected transform when selected is false', function () {
      const el = utils.buildMarkerElement({ color: '#00c896', icon: 'H', kind: 'standard', selected: false })
      const shell = el.firstChild
      expect(shell.style.transform).not.toContain('scale')
    })

    it('uses kind="standard" by default when kind is omitted', function () {
      const el = utils.buildMarkerElement({ color: '#00c896', icon: 'H' })
      expect(el.getAttribute('aria-label')).toContain('Location: H')
    })
  })
  ...
  describe('buildPopupContent', function () {
    it('returns an HTMLDivElement with title', function () {
      const el = utils.buildPopupContent('City Hospital')
      expect(el instanceof HTMLDivElement).toBe(true)
      expect(el.textContent).toContain('City Hospital')
    })

    it('includes overline when provided', function () {
      const el = utils.buildPopupContent('City Hospital', 'Hospital')
    ...
    it('renders detail rows', function () {
      const el = utils.buildPopupContent('City Hospital', null, ['Open 24/7', 'Emergency + 911'])
    ...
    it('filters out falsy detail strings while keeping valid ones', function () {
      const el = utils.buildPopupContent('City Hospital', null, ['Detail 1', '', 'Detail 3'])
