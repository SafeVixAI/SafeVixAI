...
describe('deep-link', function () {
  ...
  describe('parseDeepLink', function () {
    it('parses lat and lon correctly', async function () {
      ...
      const result = mod.parseDeepLink('https://example.com?lat=13.08&lon=80.27')
    ...
    it('returns null for invalid lat', async function () {
      const mod = await import('../deep-link')
      const result = mod.parseDeepLink('https://example.com?lat=999&lon=80.27')
    ...
    it('returns null for invalid lon', async function () {
      const mod = await import('../deep-link')
      const result = mod.parseDeepLink('https://example.com?lat=13.08&lon=999')
    ...
    it('parses state and section', async function () {
      const mod = await import('../deep-link')
      const result = mod.parseDeepLink('https://example.com?state=TN&section=MVA_185')
    ...
    it('parses sessionId', async function () {
      const mod = await import('../deep-link')
      const result = mod.parseDeepLink('https://example.com?session=abc123')
      expect(result.sessionId).toBe('abc123')
    })

    it('returns nulls for missing params', async function () {
      const mod = await import('../deep-link')
      const result = mod.parseDeepLink('https://example.com')
      expect(result.lat).toBeNull()
      expect(result.lon).toBeNull()
