...
const mockDb = { put: jest.fn(), getAllFromIndex: jest.fn(), createObjectStore: jest.fn(function() { return { createIndex: jest.fn() } }), objectStoreNames: { contains: jest.fn(function() { return false }) } }
...
const mockSupabaseAuth = require('../supabase-auth')
...
describe('chat-history', function () {
  describe('loadChatHistory', function () {
    it('returns empty array when db not available', async function () {
      ...
      const result = await (await import('../chat-history')).loadChatHistory('session-1')
  ...
  describe('appendChatLog', function () {
    it('handles db put call', async function () {
      const log = { id: '1', sessionId: 's1', role: 'user', text: 'hi', timestamp: 'now', createdAt: 'now' }
    ...
    it('skips db put when openChatDb returns null', async function () {
      mockOpenDB.mockResolvedValueOnce(null)
      mockDb.put.mockClear()
      const log = { id: '2', sessionId: 's1', role: 'user', text: 'no db', timestamp: 'now', createdAt: 'now' }
      await (await import('../chat-history')).appendChatLog(log)
    ...
    it('skips supabase insert when getSupabaseBrowserClient returns null', async function () {
      ...
      const log = { id: '3', sessionId: 's1', role: 'user', text: 'no supabase', timestamp: 'now', createdAt: 'now' }
      await (await import('../chat-history')).appendChatLog(log)
    ...
    it('skips supabase insert when offline', async function () {
      ...
      const log = { id: '4', sessionId: 's1', role: 'user', text: 'offline', timestamp: 'now', createdAt: 'now' }
      await (await import('../chat-history')).appendChatLog(log)
      expect(mockSupabase.from).not.toHaveBeenCalled()
