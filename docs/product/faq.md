...
See [SETUP.md](../developer-guide/getting_started.md) for step-by-step instructions. In short: clone the repo, set up Python virtual environments for `backend/` and `chatbot_service/`, add `.env` files, and run `npm install && npm run dev` in `frontend/`.
...
Fork the repository, connect each service to its respective platform (Vercel for frontend, Render for backend/chatbot), set environment variables, and configure the database. See [docs/Deployment.md](../developer-guide/chatbot_deployment.md).
...
Yes. The app respects GDPR/DPDP principles. See [PRIVACY.md](../spec/security/privacy_boundary.spec.md) for details on data collection, retention, and erasure policies.
...
Yes. The backend exposes REST APIs at `/api/v1/` and WebSocket endpoints for tracking. The chatbot service has its own API at `/api/v1/chat/`. See [docs/API.md](../api-reference/api_overview.md) and [docs/SDK_GUIDE.md](../api-reference/sdk_guide.md).
...
Yes. See the [Integration Guide](../developer-guide/integration_guide.md) for REST API documentation, authentication, WebSocket connections, rate limits, and SDK examples.
...
See [CONTRIBUTING.md](../developer-guide/contributing_guide.md) for contribution guidelines. We welcome bug fixes, features, documentation, tests, and translations.
...
Email **security@safevixai.gov.in**. Do not file a public issue. See [SECURITY.md](../architecture/security_overview.md) for the disclosure policy.
...
- [RUNBOOKS.md](../sre/runbooks_overview.md) — Incident response procedures
