...
Target file: [backend/.env.example](https://github.com/SafeVixAI/SafeVixAI/blob/main/backend/.env.example) -> copy to `backend/.env`
...
Target file: [frontend/.env.example](https://github.com/SafeVixAI/SafeVixAI/blob/main/backend/.env.example) -> copy to `frontend/.env`
...
| Item | Source | Local save path | Env variable name | Status | Priority |
|---|---|---|---|---|---|
| MapLibre GL JS | https://maplibre.org and npm | [frontend/package.json](https://github.com/SafeVixAI/SafeVixAI/blob/main/frontend/package.json) | N/A | Wired now | Critical |
| OpenFreeMap Liberty style | https://tiles.openfreemap.org/styles/liberty | [frontend/components/maps/MapLibreCanvas.tsx](https://github.com/SafeVixAI/SafeVixAI/blob/main/frontend/components/maps/MapLibreCanvas.tsx) and `frontend/.env` | `NEXT_PUBLIC_MAP_STYLE_URL` | Wired now | Critical |
...
| Carto Voyager backup style | https://basemaps.cartocdn.com/gl/voyager-gl-style/style.json | Future fallback in [frontend/components/maps/MapLibreCanvas.tsx](https://github.com/SafeVixAI/SafeVixAI/blob/main/frontend/components/maps/MapLibreCanvas.tsx) | N/A | Future phase | High |
| Carto Dark Matter style | Carto CDN | Future dark-mode style toggle | N/A | Future phase | Medium |
| Photon search geocoder | https://photon.komoot.io/api | [backend/core/config.py](https://github.com/SafeVixAI/SafeVixAI/blob/main/backend/core/config.py), `backend/.env` | `PHOTON_URL` | Wired now | Critical |
| Nominatim reverse geocoder | https://nominatim.openstreetmap.org | [backend/core/config.py](https://github.com/SafeVixAI/SafeVixAI/blob/main/backend/core/config.py), `backend/.env` | `NOMINATIM_URL` | Wired now | Critical |
| Overpass API | https://overpass-api.de/api/interpreter | [backend/core/config.py](https://github.com/SafeVixAI/SafeVixAI/blob/main/backend/core/config.py), `backend/.env` | `OVERPASS_URL` | Wired now | Critical |
| Overpass mirror list | `overpass.kumi.systems`, `overpass.private.coffee` | `backend/.env` | `OVERPASS_URLS` | Wired now | High |
| openrouteservice routing | https://api.openrouteservice.org | [backend/services/routing_service.py](https://github.com/SafeVixAI/SafeVixAI/blob/main/backend/services/routing_service.py), `backend/.env` | `OPENROUTESERVICE_BASE_URL`, `OPENROUTESERVICE_API_KEY` | Wired now | Critical |
...
| Gemma 4 E2B | `google/gemma-4-E2B-it` | [frontend/lib/edge-ai.ts](https://github.com/SafeVixAI/SafeVixAI/blob/main/frontend/lib/edge-ai.ts) | N/A | Future phase | Critical |
| Gemma 4 E4B | `google/gemma-4-E4B-it` | [frontend/lib/edge-ai.ts](https://github.com/SafeVixAI/SafeVixAI/blob/main/frontend/lib/edge-ai.ts) or future offline vision module | N/A | Future phase | High |
