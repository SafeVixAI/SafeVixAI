    ...
    useAppStore: function(selector) {
      ...
      if (typeof selector === 'function') {
        if (selector.toString().includes('useShallow')) {
          const shallow = require('zustand/react/shallow').useShallow
...
const mockFetchRoutePreview = jest.fn()
jest.mock('@/lib/api', () => ({
  fetchRoutePreview: function() { return mockFetchRoutePreview.apply(null, arguments) },
}))

let mockMinimumDeviation = 50
jest.mock('@/app/locator/locator-utils', () => ({
  mapService: function(service) {
    const typeMap = { hospital: 'Hospital', ambulance: 'Ambulance', police: 'Police', fire: 'Fire', towing: 'Towing' }
    const filterMap = { hospital: 'Hospital', ambulance: 'Hospital', pharmacy: 'Hospital', police: 'Police', fire: 'Fire', towing: 'Towing', puncture: 'Mechanic', showroom: 'Mechanic' }
    const colorMap = { hospital: '#ef4444', ambulance: '#10b981', police: '#3b82f6', fire: '#f97316', towing: '#f59e0b' }
    return {
...
describe('useLocatorSearch', function() {
  ...
  it('returns locating=true when no gps and no error', function() {
    ...
    const result = renderHook(function() { return require('../useLocatorSearch').useLocatorSearch() }).result
  ...
  it('auto-reroutes when gps deviates above threshold', async function() {
    const originalMinimumDeviation = mockMinimumDeviation
