...
class UpdateRelease(Base):
    ...
    signature_gpg: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    github_release_id: Mapped[int | None] = mapped_column(Integer, nullable=True, unique=True)
    github_tag_name: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class UpdateInstallation(Base):
    ...
    downloaded_bytes: Mapped[int | None] = mapped_column(Integer, nullable=True, default=0)
    total_bytes: Mapped[int | None] = mapped_column(Integer, nullable=True, default=0)
...
class UpdateSetting(Base):
    ...
    last_check_result: Mapped[str | None] = mapped_column(String(32), nullable=True)
    last_update_version: Mapped[str | None] = mapped_column(String(32), nullable=True)
    extra_data: Mapped[dict | None] = mapped_column(JSONB, nullable=True, default=dict)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=lambda: datetime.now(UTC), nullable=False
    )
