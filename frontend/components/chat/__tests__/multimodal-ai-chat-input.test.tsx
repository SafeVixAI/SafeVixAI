...
describe('PureMultimodalInput', function () {
  ...
  it('renders attachments preview with remove button', function () {
    const attachments = [{ url: 'blob:test', name: 'photo.jpg', contentType: 'image/jpeg', size: 1024 }]
  ...
  it('language menu opens and closes on globe click', function () {
    renderInput()
    const langBtn = screen.getByLabelText('chat.select_language')
  ...
  it('language selection changes displayed language', function () {
    ...
    const globeBtn = screen.getByLabelText('chat.select_language')
  ...
  it('non-image attachment shows file extension', function () {
    const attachments = [{ url: '', name: 'doc.pdf', contentType: 'application/pdf', size: 2048 }]
  ...
  it('attachment remove filters out the attachment', function () {
    const setAttachments = jest.fn()
    const attachments = [{ url: 'blob:remove-test', name: 'test.txt', contentType: 'text/plain', size: 512 }]
    renderInput({ attachments: attachments, setAttachments: setAttachments })
    expect(screen.getByTestId('input-attachment-preview')).toBeInTheDocument()
  })
  ...
  it('send button handles navigator.vibrate error gracefully', function () {
    ...
    const logClientError = require('@/lib/client-logger').logClientError
    renderInput()
    const textarea = screen.getByPlaceholderText('Ask AI anything...')
    fireEvent.change(textarea, { target: { value: 'Hello' } })
    const sendBtn = screen.getByTestId('send-button')
  ...
  it('attachments button click triggers hidden file input click', function () {
    ...
    renderInput()
    const attachBtn = screen.getByTestId('attachments-button')
  ...
  it('image attachment renders next/image component', function () {
    const attachments = [{ url: 'blob:img', name: 'photo.jpg', contentType: 'image/jpeg', size: 1024 }]
    renderInput({ attachments: attachments, setAttachments: jest.fn() })
  ...
  it('file change flow shows upload loader then resolves', async function () {
    ...
    const previews = screen.getAllByTestId('input-attachment-preview')
  ...
  it('submit form revokes blob URLs for all attachments', function () {
    const onSend = jest.fn()
    const attachments = [{ url: 'blob:test1', name: 'a.txt', contentType: 'text/plain', size: 100 }]
  ...
  it('handleRemoveAttachment revokes blob URL on X click', function () {
    const setAttachments = jest.fn()
    const attachments = [{ url: 'blob:remove-me', name: 'test.txt', contentType: 'text/plain', size: 512 }]
    renderInput({ attachments: attachments, setAttachments: setAttachments })
    const xButtons = screen.getAllByRole('button')
    const removeBtn = xButtons.find(function(b) { return b.querySelector('.lucide-x') })
  ...
  it('handleRemoveAttachment with non-blob URL does not revoke', function () {
    const setAttachments = jest.fn()
    const attachments = [{ url: 'https://example.com/img.jpg', name: 'remote.jpg', contentType: 'image/jpeg', size: 500 }]
    renderInput({ attachments: attachments, setAttachments: setAttachments })
  })
  ...
  it('speech translation failure logs warning', async function () {
    const logClientWarning = require('@/lib/client-logger').logClientWarning
  ...
  it('enter with attachments submits when input empty', function () {
    const onSend = jest.fn()
    const attachments = [{ url: 'blob:test', name: 'a.jpg', contentType: 'image/jpeg', size: 500 }]
